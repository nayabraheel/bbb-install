...
install_greenlight(){
  ...
  if [ ! -f /etc/bigbluebutton/nginx/greenlight.nginx ]; then
    docker run --rm nayabraheel/greenlight:v1 cat ./greenlight.nginx | tee /etc/bigbluebutton/nginx/greenlight.nginx
...
HERE
  ...
  if [ ! -f ~/greenlight/docker-compose.yml ]; then
    docker run --rm nayabraheel/greenlight:v1 cat ./docker-compose.yml > ~/greenlight/docker-compose.yml
  fi
